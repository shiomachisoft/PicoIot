...
**Wi-Fiモード**
* **Webブラウザでの確認:** PCやスマートフォンのブラウザからアクセスし、データを閲覧できます。表示されるデータは5秒間隔で更新されます。
...
* **TCPソケット通信:** Pico WがTCPサーバーとなり、JSON形式のデータを外部クライアント（Tera Termなど）に5秒間隔で送信します。
...
* Windows PC上で `PicoJigApp.exe` を起動します。
* Pico WをUSB接続してCOMポートを開き、通信モード（Wi-Fi/BLE）やネットワーク設定（IPアドレス、サブネットマスク、ゲートウェイ、SSID、パスワード）をFlashメモリに保存します。
<img width="502" height="492" alt="image" src="https://github.com/user-attachments/assets/4ba3b915-805e-4fe3-bf00-ea60f87918f6" />
...
<img width="452" height="743" alt="image" src="https://github.com/user-attachments/assets/49cf2cf6-26ab-47bf-9264-e0a6275c884b" />
