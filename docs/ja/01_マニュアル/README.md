...
各種センサ（GPIO入力、ADC電圧、BME280環境センサなど）から取得したデータを、Wi-FiやBLEを利用してPCやスマートフォンへ手軽に送信・表示することができます。
...
- **Wi-Fiモード**  
  ...
  - **Webブラウザでの確認:**  
    PCやスマホのブラウザからアクセスし、リアルタイムにデータを閲覧・設定変更できます。
  - **Eメール送信:**  
    指定した間隔でのデータの「**定期送信**」や、センサ値が**しきい値**を超えた際の「**アラート**通知」を行います。    
  - **TCPソケット通信:**  
    Pico WがTCPサーバーとなり、**JSON**形式のデータを外部クライアント（Tera Termなど）に連続出力します。

- **BLEモード**  
  Bluetooth Low Energy (BLE) を利用して、スマートフォンなどと直接通信するモードです。
  - Wi-Fi環境がない屋外などでも、汎用のBLEアプリを使用して手軽に**JSON**形式のセンサデータを受信・確認できます。
...
- **Eメール送信について:**  
  ...
  一般的なプロバイダのSMTPサーバーのほか、GmailのSMTPサーバーなども利用可能です。Gmailを使用する場合は、Googleアカウントのセキュリティ設定から「アプリパスワード」を生成し、Web設定画面のパスワード欄に入力してご使用下さい。
...
3. **[2]** のコンボボックスでPico WのCOM番号を選択した後、 **[3]** のボタンを押します。  
   **[4]** の表示が"connected"に変わっていればPico WとUSBで接続できています。  
   接続が完了すると、 **[5]** と **[6]** のボタンが有効になります。

### 基本設定

#### ネットワーク設定画面

ネットワーク設定画面は、[メイン画面]の **[5]** のボタンを押すと表示されます。
...
※PicoIotをもう使用しない場合は、[メイン画面]の **[6]** のボタンでFlashメモリに保存されている設定データを消去することをお勧めします。
...
画面上の「Settings」ボタンを押すと設定画面が表示されます。
...
- SMTPパスワード: Googleアカウントで生成した16桁の「アプリパスワード」  
  ※作成方法はGoogle公式ヘルプを参照して下さい。  
  （入力例: `abcd efgh ijkl mnop` ※画面で表示される通り、スペースを含めてそのまま入力して下さい）
