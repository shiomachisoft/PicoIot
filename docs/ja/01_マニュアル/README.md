...
各種センサ（GPIO入力、ADC電圧、BME280環境センサなど）から取得したデータを、Wi-FiやBLEを利用してPCやスマートフォンへ送信・表示することができます。※BME280を接続していなくても使用できます。
...
- **BLEモード**  
  BLE通信でスマホなどに**JSON**形式のデータを5秒間隔で送信します。
...
3. **[2]** のコンボボックスでPico WのCOM番号を選択した後、 **[3]** のボタンを押します。  
   **[4]** の表示が"connected"に変わっていればPico WとUSBで接続できています。  
   接続が完了すると、 **[5]** のボタン と **[6]** のボタンが有効になります。
...
- Pico Wがスマホと接続されていない場合、LEDは1000ms周期でシングルフラッシュ（1回点滅）します。
- Pico Wがスマホと接続された場合、LEDは2000ms周期でダブルフラッシュ（2回点滅）します。
...
**注意:**    
本ファームウェアはBME280のI2Cスレーブアドレスとして `0x76` を想定しています。市販のBME280モジュールを使用する場合はI2Cスレーブアドレスの設定にご注意下さい。  
