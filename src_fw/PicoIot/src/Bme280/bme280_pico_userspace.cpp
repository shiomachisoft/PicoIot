...
// @add
// =====>
struct bme280_dev g_dev = {0};
struct identifier g_id = {0};
uint8_t g_i2c_write_buf[64] = {0};
// <=====
...
int8_t user_i2c_read(uint8_t reg_addr, uint8_t *data, uint32_t len, void *intf_ptr)
{
    ...
    if ((int)len != i2c_read_blocking(id.i2c, id.dev_addr, data, len, false)) {
        return BME280_E_COMM_FAIL;
    }
    // <=====
...
int8_t user_i2c_write(uint8_t reg_addr, const uint8_t *data, uint32_t len, void *intf_ptr)
{
    ...
    if ((len + 1) > sizeof(g_i2c_write_buf)) {
        return BME280_E_COMM_FAIL;
    }

    g_i2c_write_buf[0] = reg_addr;
    memcpy(g_i2c_write_buf + 1, data, len);

    if ((int)(len + 1) != i2c_write_blocking(id.i2c, id.dev_addr, g_i2c_write_buf, len + 1, false))
    {
        return BME280_E_COMM_FAIL;
    }
...
struct bme280_dev* bme280_user_init(void)
{
    ...
    g_id.i2c = BME280_USER_I2C_ID;

    /* Make sure to select BME280_I2C_ADDR_PRIM or BME280_I2C_ADDR_SEC as needed */
    g_id.dev_addr = BME280_USER_SLAVE_ADDR;
